$(function () {
    $('.widge_tags li a').tooltip({container: 'body'})
    $('.w-readers').tooltip({selector: "[data-toggle=tooltip]", container: "body"})
    $('.favorite').tooltip()
    $('.social_a a').tooltip({container: "body"})
    $('.author i').tooltip({container: "body"})
    $('.weibo a').tooltip({container: "body"})
    $('.mail a').tooltip({container: "body"})
    $('.postauthor-right .name').tooltip({container: "body"})
})
$(function () {
    bodyh = $(window).height();
    bodyw = $(window).width();
    $(".share-item").css({"top": bodyh / 3 + "px", "left": bodyw / 2 - 140 + "px",});
...
jQuery(document).ready(function ($) {
    var $sitetopbar = $('#sitetopbar'), timeout = false;
