...
async def hot_posts(self,post_type=0):
    ...
    hot_posts = await self.application.db.posts.find({'post_date': {'$gte': one_day_ago},"type":post_type},{ "_id": 1,"title": 1 ,"content":1}).sort([("views", -1)]).limit(hot_news_num).to_list(length=hot_news_num)
    for x in hot_posts:
        x['_id'] = str(x['_id'])
...
async def new_comment_posts(self,post_type=0):
    new_comment_posts_ids = await self.application.db.comments.aggregate([
        {
            "$group":
                {
                    "_id": "$post_id",
    ...
    new_comment_posts =  await self.application.db.posts.find({'_id': {'$in': new_comment_posts_ids}}).to_list(length=None)
...
async def c_hot_posts(self,c_id):
    one_day_ago = datetime.datetime.now() - datetime.timedelta(days=1)
    hot_posts_list = await self.application.db.posts.find({'post_date': {'$gte': one_day_ago},"type":0,"category":ObjectId(c_id)},{ "_id": 1,"title": 1 }).sort([("views", -1)]).limit(hot_news_num).to_list(length=hot_news_num)
    for x in hot_posts_list:
        x['_id'] = str(x['_id'])
    hot_posts = dict({"name":await get_tname_by_tid(self.application.db,c_id)})
...
async def u_new_posts(self,u_id,post_type=0):
    u_new_posts = await self.application.db.posts.find({ "user": u_id,"type":post_type},{ "_id": 1,"title": 1 }).limit(5).to_list(length=5)
...
async def u_categorys(self,u_id):
    u_categorys = await self.application.db.terms.find({ "user": u_id,"type":"2"},{ "_id": 1,"name": 1 }).to_list(length=None)
