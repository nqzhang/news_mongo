...
class NewPostHandler(RequestHandler):
    ...
    async def post(self):
        ...
        try:
            #插入用户
            u = await self.db.users.find_one_and_update({"user_name":user},
            ...
            for c_name in category:
                c = await self.db.terms.find_one_and_update({"name":c_name,"type":"0"},
            ...
            t_ids = []
            for t_name in tags:
                t = await self.db.terms.find_one_and_update({"name": t_name, "type": "1"},
                                                                {"$setOnInsert": {"type": "1", "name": t_name}}, upsert=True,return_document=ReturnDocument.AFTER)
                t_ids.append(t['_id'])
            print(t_ids)

            #插入文章
            await self.db.posts.find_one_and_update({"title": title},
