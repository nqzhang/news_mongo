...
class recommendPageHandler(BaseHandler):
    async def get(self,page=1):
        ...
        posts =  await self.application.db.posts.find({"is_real_user": 1}).sort([("post_date",-1)]).skip(articles_per_page * (int(page) - 1)).limit(articles_per_page).to_list(length=articles_per_page)
