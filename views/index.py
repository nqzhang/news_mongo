...
class IndexPageHandler(BaseHandler,DBMixin):
    async def get(self,page=1):
        ...
        posts =  await self.application.db.posts.find({"type":0}).sort([("post_date",-1)]).skip(self.articles_per_page * (int(page) - 1)).limit(self.articles_per_page).to_list(length=self.articles_per_page)
